...
const std::string SatoGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SatoGUI::DEFAULT_WALLET = "~Default";

SatoGUI::SatoGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void SatoGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Sato address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and sato: URIs)"));
    ...
    /** SATO START */
    transferAssetAction = new QAction(QIcon(":/icons/asset_transfer"), tr("&Transfer Assets"), this);
    transferAssetAction->setStatusTip(tr("Transfer assets to SATO addresses"));
    ...
    createAssetAction = new QAction(QIcon(":/icons/asset_create"), tr("&Create Assets"), this);
    ...
    manageAssetAction = new QAction(QIcon(":/icons/asset_manage"), tr("&Manage Assets"), this);
    ...
    messagingAction = new QAction(QIcon(":/icons/editcopy"), tr("&Messaging"), this);
    ...
    votingAction = new QAction(QIcon(":/icons/edit"), tr("&Voting"), this);
    ...
    restrictedAssetAction = new QAction(QIcon(":/icons/asset_restricted"), tr("&Restricted Assets"), this);
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Sato addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Sato addresses"));
    ...
    openAction->setStatusTip(tr("Open a sato: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Sato command-line options").arg(tr(PACKAGE_NAME)));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* satoLabelLayout = new QVBoxLayout(toolbarWidget);
        satoLabelLayout->addWidget(label);
        satoLabelLayout->addWidget(toolbar);
        satoLabelLayout->setDirection(QBoxLayout::TopToBottom);
        satoLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Sato Market Price"));
        ...
        // labelVersionUpdate->setText("<a href=\"https://github.com/SatoProject/Sato/releases\">New Wallet Version Available</a>");
        ...
        labelCurrentPrice->setText("0.00000000");
        ...
        // Get the latest Sato release and let the user know if they are using the latest version
        // Network request code for the header widget
        ...
        //                            if (fRet) {
        //                                QString link = "https://github.com/SatoProject/Sato/releases";
...
void SatoGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SatoGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SatoGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SatoGUI::setWalletActionsEnabled(bool enabled)
{
...
void SatoGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SatoGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SatoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SatoGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SatoGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void SatoGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SatoGUI::openClicked()
{
...
void SatoGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SatoGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SatoGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SatoGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SatoGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SatoGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** SATO START */
void SatoGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void SatoGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void SatoGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void SatoGUI::gotoRestrictedAssetsPage()
{
...
void SatoGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Sato network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void SatoGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SatoGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SatoGUI::updateHeadersSyncProgressLabel()
{
...
void SatoGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SatoGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Sato"); // default title
...
void SatoGUI::changeEvent(QEvent *e)
{
...
void SatoGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SatoGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SatoGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "SATO")
        msg += tr("Amount: %1\n").arg(SatoUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(SatoUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void SatoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SatoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SatoGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SatoGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SatoGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void SatoGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SatoGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SatoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SatoGUI::detectShutdown()
{
...
void SatoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SatoGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SatoGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SatoGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(SatoGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void SatoGUI::subscribeToCoreSignals()
{
...
void SatoGUI::unsubscribeFromCoreSignals()
{
...
void SatoGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SatoGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SatoUnits::Unit> units = SatoUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SatoUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(SatoUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(SatoUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (SatoUnits::Unit u : SatoUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SatoUnits::name(u)), this);
...
{
    setText(SatoUnits::name(newUnits));
}
...
void SatoGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/SatoProject/Sato/releases"));
